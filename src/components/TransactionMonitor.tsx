    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Real-time Transaction Monitor</h2>
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            ...
            <span>{viewMode === 'compact' ? 'Expand' : 'Compact'}</span>
          </button>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900 dark:to-blue-800 p-6 rounded-xl border border-blue-200 dark:border-blue-700 transform hover:scale-105 transition-all duration-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-blue-600 dark:text-blue-400 font-medium">Total Transactions</p>
                <p className="text-3xl font-bold text-blue-900 dark:text-blue-100">{stats.totalTransactions}</p>
              </div>
          ...
          <div className="bg-gradient-to-br from-red-50 to-red-100 dark:from-red-900 dark:to-red-800 p-6 rounded-xl border border-red-200 dark:border-red-700 transform hover:scale-105 transition-all duration-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-red-600 dark:text-red-400 font-medium">Malicious Blocked</p>
                <p className="text-3xl font-bold text-red-900 dark:text-red-100">{stats.maliciousBlocked}</p>
              </div>
          ...
          <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 dark:from-yellow-900 dark:to-yellow-800 p-6 rounded-xl border border-yellow-200 dark:border-yellow-700 transform hover:scale-105 transition-all duration-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-yellow-600 dark:text-yellow-400 font-medium">Avg Risk Score</p>
                <p className="text-3xl font-bold text-yellow-900 dark:text-yellow-100">{stats.averageRiskScore}%</p>
              </div>
          ...
          <div className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900 dark:to-green-800 p-6 rounded-xl border border-green-200 dark:border-green-700 transform hover:scale-105 transition-all duration-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-green-600 dark:text-green-400 font-medium">Throughput (TPS)</p>
                <p className="text-3xl font-bold text-green-900 dark:text-green-100">{stats.throughput}</p>
              </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Risk Heatmap (Last 50 Transactions)</h3>
        <div className="flex space-x-1 overflow-x-auto">
          {riskHeatmapData.map((item, index) => (
            <div
              key={index}
              className={`w-3 h-8 ${item.color} rounded-sm opacity-80 hover:opacity-100 transition-opacity cursor-pointer`}
            ...
            <div className="divide-y divide-gray-100 dark:divide-gray-700">
              {filteredTransactions.map((tx, index) => (
                <div 
                  ...
                  className={`p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-200 border-l-4 animate-in slide-in-from-left duration-500 ${
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-3 mb-2">
                        ...
                        {isWalletBlocked(tx.from) && (
                          <span className="px-2 py-1 text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300 rounded-full animate-pulse">
                            BLOCKED WALLET
                          </span>
                        )}
                      </div>
                      ...
                      {viewMode === 'expanded' && (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600 dark:text-gray-400 mb-2">
                          ...
                          <div>
                            <span className="font-medium">From:</span> {tx.from.substring(0, 8)}...
                          </div>
                          <div>
                        ...
                        <div className="mt-2">
                          <div className="flex flex-wrap gap-1">
                            {tx.analysis.detectedPatterns.map((pattern, patternIndex) => (
                              <span key={patternIndex} className="px-2 py-1 text-xs bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300 rounded animate-pulse">
                                {pattern}
                              </span>
                            ))}
                          </div>
                    ...
                    <div className="text-right text-sm text-gray-500 dark:text-gray-400">
                      ...
                      <div className="font-mono">{new Date(tx.timestamp).toLocaleTimeString()}</div>
